...
import "./painting-recipe-compact.css";
...
export function PaintingRecipeSheet({ lesson, actions, presentation = "standard" }: { lesson: PaintingLesson; actions?: ReactNode; presentation?: "standard" | "compact" }) {
  ...
  const artwork = painting ? <img src={painting.image_url} width={painting.width} height={painting.height} alt={lesson.is_demo ? "Your original inspiration" : "Simple finished watercolor painting"} /> : <p>Your painting image is unavailable.</p>;
  return <section className={`recipe-sheet${compact ? " recipe-sheet--compact" : ""}`} aria-label="Simple painting recipe">
    <header className="recipe-sheet__heading"><div>{!compact && <p className="eyebrow">A little time to paint</p>}<h1>{lesson.title}</h1></div><TracingControls url={outline?.image_url} /></header>
    ...
    <div className="recipe-sheet__workspace"><div className="recipe-sheet__art">
      <figure>{compact && painting ? <button type="button" className="recipe-sheet__enlarge" aria-label="Enlarge painting" aria-haspopup="dialog" onClick={() => setEnlarged(true)}>{artwork}<span>Enlarge painting ↗</span></button> : artwork}{lesson.is_demo && <figcaption>Your inspiration</figcaption>}</figure>
    </div>
    ...
    <nav className="recipe-sheet__tools" aria-label="Recipe actions">{actions ?? <><Link className="text-link" to={`/?view=lesson-target&lesson=${lesson.id}`}>Try a different painting</Link><Link className="text-link" to="/?view=sessions">Your painting sessions</Link></>}</nav>
    {compact && painting && <StudioDialog open={enlarged} onClose={() => setEnlarged(false)} wide label="Enlarged painting" className="garden-dialog recipe-art-dialog"><h2>{lesson.title}</h2><img src={painting.image_url} width={painting.width} height={painting.height} alt="Enlarged painting reference" /></StudioDialog>}
  </section>;
}
