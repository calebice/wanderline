...
export function PaintingRecipeSheet({ lesson, actions, presentation = "standard" }: { lesson: PaintingLesson; actions?: ReactNode; presentation?: "standard" | "compact" }) {
  ...
  return <section className={`recipe-sheet${compact ? " recipe-sheet--compact" : ""}`} aria-label="Simple painting recipe">
    ...
    <nav className="recipe-sheet__tools" aria-label="Recipe actions">{actions ?? <><Link className="text-link" to={`/sessions/${lesson.id}/target`}>Try a different painting</Link><Link className="text-link" to="/sessions">Your painting sessions</Link></>}</nav>
