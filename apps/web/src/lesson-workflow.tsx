  ...
  return <article className="lesson-creator">
    ...
    <form onSubmit={submit}>
      <fieldset className="creator-fields" disabled={status === "uploading"}>
        <section aria-label="Your inspiration"><p className="creator-inspiration-note">Start with something that catches your eye. A photo or a few words is enough.</p><div className="lesson-source-tabs"><button type="button" className={brief.source_mode === "upload" ? "is-selected" : ""} aria-pressed={brief.source_mode === "upload"} onClick={() => setBrief({ ...brief, source_mode: "upload", scene_prompt: null })}>Use a photo</button><button type="button" className={brief.source_mode === "prompt" ? "is-selected" : ""} aria-pressed={brief.source_mode === "prompt"} disabled={imageAvailable !== true} onClick={() => setBrief({ ...brief, source_mode: "prompt" })}>Describe an idea</button></div>
        ...
        <div className="creator-basics"><p id="creator-defaults" className="creator-defaults">{brief.sequence_style === "simple_recipe" ? "One subject · a few colors · one printable page" : "3 steps · 30 minutes · easygoing watercolor"}</p><button type={customizing ? "button" : "submit"} className="lesson-generate" aria-describedby="creator-defaults creator-preview-note" onClick={customizing ? () => void generate(true) : undefined}>{status === "uploading" ? "Preparing your inspiration…" : "Just stick to basics"}</button><p id="creator-preview-note">We’ll prepare a preview for you to approve before building your lesson.</p></div>
        <button type="button" className="creator-customize button-secondary" aria-expanded={customizing} aria-controls="creator-options" onClick={() => setCustomizing(!customizing)}>{customizing ? "Close options" : "Make it yours"}</button>
        {customizing && <section id="creator-options" className="creator-options"><p className="creator-options-note">Optional: shape the direction of your preview.</p>{brief.sequence_style !== "simple_recipe" && <><ArtDirectionControls brief={brief} onChange={setBrief} /><SequenceStyleControls brief={brief} onChange={setBrief} /></>}<details className="lesson-more-options"><summary>A few more touches</summary><div className="lesson-form-grid"><label>Session title<input value={title} maxLength={200} onChange={(event) => setTitle(event.target.value)} placeholder="We can suggest one" /></label><label>How much guidance?<select value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><option value="beginner">A gentle start</option><option value="intermediate">A little experience</option><option value="advanced">Room to stretch</option></select></label><label>Time to paint<select value={duration} onChange={(event) => setDuration(Number(event.target.value))}>{[20, 30, 45, 60].map((time) => <option key={time} value={time}>{time} minutes</option>)}</select></label><label className="is-wide">Anything you’d like to explore?<textarea value={context} maxLength={1000} onChange={(event) => setContext(event.target.value)} /></label></div></details></section>}
      </fieldset>
      {error && <div className="lesson-error" role="alert">{error}</div>}
      {customizing && <div className="studio-actions creator-submit"><button className="button-secondary" type="submit" disabled={status === "uploading"}>{status === "uploading" ? "Preparing your inspiration…" : "Create my preview →"}</button></div>}
